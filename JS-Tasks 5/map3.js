...
function stringItUp(arr){
  return arr.map(num => num = String(num));
  // your code here
}
console.log(stringItUp([2, 5, 100]));
...
console.log(capitalizeNames(["john","razan", "JACOB", "jinGleHeimer", "schmidt"])); 
...
function namesOnly(arr){
  return arr.map( (currentName) => currentName.name );
}
...
function makeStrings(arr){
  return arr.map(currentName => {
    if(currentName.age > 50)
    {
      return `${currentName.name} can go to the matrix`
    }
    else 
    return `${currentName.name} is under the age!!`
...
function readyToPutInTheDOM(arr){
  return arr.map(currentName => 
  {
    return `<h1>${currentName.name}</h1><h2>${currentName.age}<h2>`
  });
...
function doubleValues(arr){
  ...
  */
  ...
  function extractKey(arr, k){
    let newArray = arr.map(currentName => {
 ...
 function extractFullName(arr){
  return arr.map(currentElement => {
    return String(currentElement.first).concat(" ").concat(currentElement.last);
  });
 }
